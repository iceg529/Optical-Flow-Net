...
function CorrelationLayer:updateOutput(input)
   self.tempIn1 = torch.CudaTensor(input[1]:size(1),(input[1]:size(2))+20,(input[1]:size(3))+20):fill(0)
   self.tempIn2 = torch.CudaTensor((self.tempIn1):size()):fill(0)
   -- fill the original area with original values
   self.tempIn1:sub(1,input[1]:size(1),11,10+input[1]:size(2),11,10+input[1]:size(3)):copy(input[1])
   self.tempIn2:sub(1,input[2]:size(1),11,10+input[2]:size(2),11,10+input[2]:size(3)):copy(input[2]) 
   self.output = torch.CudaTensor(self.pSize,input[1]:size(2),input[1]:size(3))
   for i=1,input[1]:size(2) do
     for j=1,input[1]:size(3) do
	(self.patch1)[1] = input[1][{{},i,j}]
	self.patch2 = torch.reshape(((self.tempIn2):sub(1,self.inSize,i,i+20,j,j+20)), self.pSize, self.inSize)
...
function CorrelationLayer:updateGradInput(input, gradOutput)
   self.tempGrad = torch.CudaTensor(gradOutput:size(1),(gradOutput:size(2))+20,(gradOutput:size(3))+20):fill(0)
   -- fill the original area with original values
   self.tempGrad:sub(1,gradOutput:size(1),11,10+gradOutput:size(2),11,10+gradOutput:size(3)):copy(gradOutput)
   
   self.gradInput = torch.CudaTensor(2,input[1]:size(1),input[1]:size(2),input[1]:size(3))
   for i=1,input[1]:size(2) do
     for j=1,input[1]:size(3) do
  	self.subGrad = torch.reshape(((self.tempIn2):sub(1,input[1]:size(1),i,i+20,j,j+20)),self.pSize, self.inSize)
	self.gradPatch[1] = gradOutput[{{},i,j}]
  	self.gradInput[1][{{},i,j}] = ((torch.mm(self.gradPatch, self.subGrad))[1])
  	
	self.subGrad = torch.reshape(((self.tempIn1):sub(1,input[1]:size(1),i,i+20,j,j+20)),self.pSize, self.inSize)
	self.gradPatch[1] = (torch.diag(torch.reshape(((self.tempGrad):sub(1,gradOutput:size(1),i,i+20,j,j+20)),self.pSize, self.pSize)))
   ...
   --self.gradInput:div(gradOutSize)
